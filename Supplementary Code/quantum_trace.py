...
frozen = w90(r".", r"wannier90")
my_model = frozen.model(min_hopping_norm=0.00001)
...
"""This is the code to calculate the Berry curvature and Wilson loop of a tight-binding model with built-in functions.
my_array_1=wf_array(my_model_reduced,[50, 50])
# solve model on a regular grid, and put origin of
# Brillouin zone at -1/2 -1/2 point
my_array_1.solve_on_grid([0.0,0.0])
# Berry flux for lower band
flux_a_1=my_array_1.berry_flux([3])

# also plot Berry phase on each small plaquette of the mesh
plaq=my_array_1.berry_flux([3],individual_phases=True)
#
fig, ax = plt.subplots()
ax.imshow(plaq.T)
...
my_array_1=wf_array(my_model_reduced,[51,51])
my_array_1.solve_on_grid([0, 0])
phi_1 = my_array_1.berry_phase([4],0,contin=False,berry_evals=True)
...
def berry_curvature(b1, b2, steps, target_band):
    ...
    start_point = (np.array(b1) - np.array(b2)) / 2
    ...
    ax.axis('equal')
    plt.show()
    fig.savefig('berry_curvature_valence.eps',ppi=400)
    return chern_num, fluctuation_berry


if __name__ == '__main__':
    # band_structure(k_path, k_label)
    a, b = berry_curvature([0.07975, 0.04604, 0.     ], [0.,0.09209,0.     ], 50, [11])
